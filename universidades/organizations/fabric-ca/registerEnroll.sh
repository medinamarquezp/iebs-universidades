...
function createBerlin() {
  ...
  infoln "Generating the user msp"
  set -x
  fabric-ca-client enroll -u https://user1:user1pw@localhost:10054 --caname ca-berlin -M "${PWD}/organizations/peerOrganizations/berlin.universidades.com/users/User1@berlin.universidades.com/msp" --tls.certfiles "${PWD}/organizations/fabric-ca/berlin/tls-cert.pem"
  { set +x; } 2>/dev/null
